...
from TelethonNobi.utils.globals import CMD_LIST, LOAD_PLUG
    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            nobi_reg = sudo_reg = re.compile(pattern)
        else:
            nobi_ = "\\" + Config.HANDLER
            sudo_ = "\\" + Config.SUDO_HANDLER
            nobi_reg = re.compile(nobi_ + pattern)
    ...
    def decorator(func):
        if not disable_edited:
            Nobi.add_event_handler(
                func, events.MessageEdited(**args, outgoing=True, pattern=nobi_reg)
            )
        Nobi.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=nobi_reg)
        )
        if allow_sudo:
            if not disable_edited:
                Nobi.add_event_handler(
