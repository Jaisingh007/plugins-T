...
def validate_session(session):
    if "==nobi" and "bot==" in session.lower():
        new_session = session[6:-5]
        return str(new_session)
    else:
        print(f"NobiUb SESSION - Wrong session string!")
        sys.exit()


if Config.NobiUb_SESSION:
    session = StringSession(validate_session(Config.NobiUb_SESSION))
else:
    session = "NobiUb"
