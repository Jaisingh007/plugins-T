...
from TelethonNobi.clients.session import H2, H3, H4, H5, Nobi
from TelethonNobi.utils.globals import CMD_LIST, LOAD_PLUG, SUDO_LIST
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                nobireg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                nobireg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(nobireg + pattern)
    ...
    # blacklisted chats.
    # NobiUb will not respond in these chats.
...
def on(**args):
    def decorator(func):
        ...
        Nobi.add_event_handler(wrapper, events.NewMessage(**args))
        if H2:
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Nobi.add_event_handler(func, events.MessageEdited(**args))
        Nobi.add_event_handler(func, events.NewMessage(**args))
        if H2:
