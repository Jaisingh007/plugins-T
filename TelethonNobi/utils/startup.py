...
from TelethonNobi.helpers.int_str import make_int
from TelethonNobi.version import __telever__


# Creates the logger group on first deploy and adds the helper bot
async def logger_id(client):
    desc = "A Bot Logger Group For NobiUb. DO NOT LEAVE THIS GROUP!!"
    try:
        grp = await client(
            functions.channels.CreateChannelRequest(
                title="NobiUb Logger", about=desc, megagroup=True
            )
        )
...
<b><i>»» <u><a href='https://t.me/'>ᴛʜᴇ ɴᴏʙɪ ᴜʙ</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("NobiUb Network", "https://t.me/the_NobiUb")]],
    )


# Joins the NobiUb chat and channel from all clients
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@the_NobiUb"))
