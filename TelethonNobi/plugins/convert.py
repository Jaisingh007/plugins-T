...
async def _(event):
    ...
    if type_ == 'Sticker':
        nobi_ = await event.client.download_media(rply.media)
        gifs = await tgs_to_gif(nobi_, True)
        unsave =  await event.client.send_file(event.chat_id, file=gifs, force_document=False)
        await unsave_gif(event, unsave)
        await nobi.delete()
        os.remove(nobi_)
        os.remove("NobiUb.gif")
    else:
        await parse_error(nobi, "Only animated stickers are supported.")


@nobi_cmd(pattern="stoi$")
async def _(nobi):
    reply_to_id = nobi.message.id
    if nobi.reply_to_msg_id:
        reply_to_id = nobi.reply_to_msg_id
    event = await eor(nobi, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await nobi.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            await nobi.client.send_file(
                event.chat_id,
...
async def _(event):
    ...
    try:
        ...
        await nobi.edit(f"__Downloaded:__ `{downloaded_file_name}` \n__Time taken:__ `{ms} seconds`")
        ...
        if input_str == "voice":
            ...
            await parse_error(nobi, "Not supported")
            os.remove(downloaded_file_name)
            return
        ...
        if os.path.exists(new_required_file_name):
            ...
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, nobi, c_time, "Uploading ...")
                ),
            )
