...
async def install(event):
    ...
    nobi_ = await eor(event, "__Installing plugins from Plugin Channel...__")
    nobi = "**⦁ Installed Plugins:**\n\n"
    for ixo in total_doxx:
        mxo = documentss[ixo].id
        downloaded_file_name = await event.client.download_media(
            await event.client.get_messages(chat, ids=mxo), "TelethonNobi/plugins/"
        )
        if "(" not in downloaded_file_name:
            path1 = Path(downloaded_file_name)
            shortname = path1.stem
            load_module(shortname.replace(".py", ""))
            nobi += "__» Installed Plugin__ `{}`\n".format(
                os.path.basename(downloaded_file_name)
            )
        else:
            nobi += "__» Failed to Install__ `{}`\n".format(
                os.path.basename(downloaded_file_name)
            )
    await nobi_.edit(nobi)


@nobi_cmd(pattern="installall ([\s\S]*)")
async def install(event):
    chat = event.pattern_match.group(1)
    nobi_ = await eor(event, f"**Starting To Install Plugins From {chat} !!**")
    nobi = f"**⦁ Installed Plugins From {chat} :**\n\n"
    documentss = await event.client.get_messages(
        chat, None, filter=InputMessagesFilterDocument
    )
    total = int(documentss.total)
