...
async def _(event):
    ...
    if len(lists) == 3:
        ...
        if stripped.startswith("-s"):
            ...
            url = stripped
    _, _, nobi_memtion = await client_id(event)
    reply = await event.get_reply_message()
    if reply:
        nobi = await eor(event, "`Analysing...`")
        file_name = "rmbg.png"
        try:
            ...
            await nobi.edit("`Removing background of this media`")
            file_name = toimage(file_name, "temp.jpeg")
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif url:
        nobi = await eor(event, "`Removing Background of this media`")
    ...
    remove_bg_image = "NobiUb.png"
    if "image" in contentType:
        with open("NobiUb.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        return await parse_error(nobi, response.content.decode('UTF-8'))
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="NobiUb.webp")
        ...
        await event.client.send_file(
            event.chat_id,
            file,
            caption=f"**Background removed by** {nobi_memtion}",
            force_document=True,
        )
    else:
