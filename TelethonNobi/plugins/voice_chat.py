...
@nobi_cmd(pattern="vcinvite$")
async def _(event):
    nobi = await eor(event, "`🧐 Inviting Users To Voice Chat....`")
    users = []
    i = 0
    ...
    for k in lists:
        try:
            ...
            pass
    await parse_error(nobi, f"**🚀 Invited** `{i}` **users to Voice Chat.**")
