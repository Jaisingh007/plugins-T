...
msg = f"""
**⚡ ʟɛɢɛռɖaʀʏ ᴀғ 𝕹𝕺𝕭𝕴 𝖀𝕭 ⚡**
  •        [📑 Repo 📑](https://github.com/jaisingh007/THE-NOBI_UB)
  •        [NobiUb Network](https://t.me/THE_NOBIUB)
  •  ©️ {nobi_channel} ™
"""


@nobi_cmd(pattern="repo$")
async def repo(event):
    ForGo10God, _, _ = await client_id(event)
    try:
        nobi = await event.client.inline_query(Config.BOT_USERNAME, "repo")
...
async def _(event):
    if Config.BOT_USERNAME:
        try:
            ...
            nobi = await eor(event, "**Inline Mode is disabled.** \n__Turning it on, please wait for a minute...__")
            async with bot.conversation("@BotFather") as conv:
                try:
                    ...
                    return await parse_error(nobi, "__Unblock__ @Botfather __first.__", False)
                await eod(nobi, f"**Turned On Inline Mode Successfully.** \n\nDo `{hl}help` again to get the help menu.")
...
@nobi_cmd(pattern="plinfo(?:\s|$)([\s\S]*)")
async def NobiUbt(event):
...
@nobi_cmd(pattern="cmdinfo(?:\s|$)([\s\S]*)")
