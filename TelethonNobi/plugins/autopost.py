...
async def _(event):
    ...
    if str(hel_).startswith("-100"):
        ...
        kk = hel_
    if not kk.isdigit():
        return await parse_error(nobi, "Only channel ID is supported.")
    if is_post(kk, event.chat_id):
        if checker and checker == "True":
            return await eod(nobi, "This channel is already in this client's autopost database.")
        else:
    ...
    addgvar(f"AUTOPOST_{str(cli_)}", "True")
    await nobi.edit(f"**📍 Started AutoPosting from** `{hel_}` for `{cli_}`")


@nobi_cmd(pattern="rmautopost(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
        return await parse_error(event, "AutoPost Can Only Be Used For Channels.")
    nobi = await eor(event, "Removing autopost...")
    cid = await client_id(event)
    ForGo10God = cid[0]
    hel_ = event.text[12:]
    cli_ = ForGo10God
    ...
    if is_post(kk, event.chat_id):
        if checker and checker == "True":
            ...
            return await eod(nobi, f"Removed `{hel_}` from `{cli_}` autopost database.")
        else:
            return await eod(
                nobi, f"This channel is not in `{cli_}` autopost database."
            )
