...
<b>⚡ <a href='https://t.me/NobiUb_Network'>From DataBase of NobiUb</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@nobi_cmd(pattern="chatinfo(?:\s|$)([\s\S]*)")
async def info(event):
    nobi = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await nobi.edit(caption, parse_mode="HTML")
    except Exception as e:
        print("Exception:", e)
        await parse_error(nobi, "An unexpected error has occurred.")
    return
...
@nobi_cmd(pattern="id$")
async def _(event):
    nobi = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await nobi.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
