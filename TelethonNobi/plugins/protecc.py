...
from TelethonNobi.DB.husb_sql import (add_hus_grp, get_all_hus_grp, is_husb,
                                      rm_hus_grp)
from TelethonNobi.DB.waifu_sql import add_grp, get_all_grp, is_harem, rm_grp
...
@nobi_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
    BASE_URL = "http://images.google.com"
    if event.reply_to_msg_id:
        nobi = await eor(event, "Hmm..")
        ...
        await nobi.edit(f"/protecc {prs_text}", parse_mode="HTML", link_preview=False)
    else:
...
async def _(event):
    ...
    if is_harem(str(event.chat_id)):
        try:
            ...
            alls = div.find("a")
            text = alls.text
            nobi = await event.client.send_message(
                event.chat_id,
                f"/protecc {text}",
                reply_to=event,
            )
...
@nobi_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@nobi_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, nobi_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autowaifu works in groups only !!")
...
@nobi_cmd(pattern="autowaifu$")
async def _(event):
    nobi = await eor(event, "Fetching Autowaifu chats...")
    all_grp = get_all_grp()
...
@nobi_cmd(pattern="autohusbando$")
async def _(event):
    nobi = await eor(event, "Fetching Autohusbando chats...")
