...
async def download(event):
    ...
    splt = txts.split("|")
    path = splt[0]
    branch = splt[1] or "nobi"
    NobiUb = await eor(event, "Processing ...")
    ...
    try:
        ...
        await NobiUb.edit("Committing to Github....")
        await git_commit(downloaded_file_name, path, branch, NobiUb)


async def git_commit(file_name, path, branch, NobiUb):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await NobiUb.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            repo.create_file(
                path,
                f"Uploaded file {file_name} by 𝕹𝕺𝕭𝕴 𝖀𝕭✍",
                commit_data,
                branch=branch,
            )
            ...
            await parse_error(NobiUb, "Cannot Upload File")
    else:
        return await parse_error(NobiUb, "Committed Suicide")
...
CmdHelp("github").add_command(
    "commit", "<reply to a file> <path>|<branch>", "Uploads the file on github repo as provided in Heroku Config GIT_REPO_NAME. In short makes a commit to git repo from Userbot", "commit ./NobiUb/plugins/example.py|nobi"
).add_command(
