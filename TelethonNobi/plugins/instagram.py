...
async def download(event):
    ...
    nobi = await eor(event, "IG downloader in action...")

    if flag.lower() == "-htag":
        if link and link.startswith("#"):
            await nobi.edit(f"IG downloader in action... \n\nUploading top {post} posts of `{link}`")
            try:
                    ...
                    for single in items_list:
                        try:
                            await event.client.send_file(
                                event.chat_id, 
                                single,
                                caption=f"📥 InstaGram Post Downloaded By :- {nobi_mention}",
                            )
                            count += 1
                        except Exception as e:
                            LOGS.info(str(e))
                        os.remove(single)
                await nobi.edit(f"**Downloaded top posts of** `{link}` \n\n__Total:__ `{count} posts.`")
            except Exception as e:
                return await parse_error(nobi, e)
        else:
            return await eod(nobi, f"**SYNTAX EXAMPLE:** \n\n`{hl}igdl -htag #amvs`\n\nThis will give top 10 IG posts of hashtag `#amvs`.")
    elif link:
        ...
        try:
            ...
            return await parse_error(nobi, e)
        await nobi.edit("**Downloaded post.** __Uploading now ...__")
        items_list = [os.path.join('./insta/dl', file) for file in os.listdir('./insta/dl')]
        count = 0
        if items_list != []:
            for single in items_list:
            ...
            await nobi.edit(f"**Downloaded Instagram Post!** \n\n__Total:__ `{count} posts.`")
        else:
            await parse_error(nobi, "Unable to upload video! Check LOGS and try again!")
    else:
        await parse_error(nobi, "Give proper command.")


@nobi_cmd(pattern="igup(?:\s|$)([\s\S]*)")
async def upload(event):
    flag, url = await get_flag(event)
    nobi = await eor(event, "IG uploader in action...")
    reply = await event.get_reply_message()
    caption = f"{reply.message} \n\n" if reply.message else ""
    caption += "• #UploadedByNobiUb"
    nobi_MEDIA = media_type(reply)
    ...
    if not reply:
        return await eod(nobi, "Reply to a media to upload on instagram.")
    if not reply.media:
        return await eod(nobi, "Reply to a media to upload on instagram.")

    if flag.lower() == "-reel":
        if nobi_MEDIA not in ["Gif", "Video"]:
            return await eod(nobi, "A reel can only be GIF or Video!")
        file = await event.client.download_media(reply)
        await nobi.edit("**Downloaded!** \n\nNow uploading reel to instagram...")
        try:
            video = IG.clip_upload(path=file, caption=caption)
        except Exception as e:
            os.remove(file)
            return await parse_error(nobi, e)
        await nobi.edit(f"**Uploaded Reel to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)
    
    elif flag.lower() == "-tv":
        if nobi_MEDIA not in ["Gif", "Video"]:
            return await eod(nobi, "An IGTV can only be GIF or Video!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(nobi, e)
        await nobi.edit(f"**Uploaded IGTV to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)
    
    elif flag.lower() == "-vid":
        if nobi_MEDIA not in ["Gif", "Video"]:
            return await eod(nobi, "A video post can only be GIF or Video!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(nobi, e)
        await nobi.edit(f"**Uploaded Video to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)

    elif flag.lower() == "-pic":
        if nobi_MEDIA != "Photo":
            return await eod(nobi, "A picture post can only be a Photo!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(nobi, e)
        await nobi.edit(f"**Uploaded Photo to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)

    elif flag.lower() == "-story":
        if nobi_MEDIA in ["Gif", "Video"]:
            file = await event.client.download_media(reply)
            ...
            return await parse_error(nobi, "Invalid media format. Only Videos, Pictures, GIF are supported to upload story.")

    else:
        await eod(nobi, f"Give proper flag. Check `{hl}plinfo instagram` for details.")
    

@nobi_cmd(pattern="iguser(?:\s|$)([\s\S]*)")
async def userinfo(event):
    uname = event.text.split(" ", 2)[1]
    username = uname.replace("@", "") if "@" in uname else uname
    nobi = await eor(event, f"Searching `{username}` on Instagram...")
    info_str = """
...
"""
    ...
    if IG:
        ...
        await parse_error(nobi, "`INSTAGRAM_SESSION` __not configured or Expired !__", False)
