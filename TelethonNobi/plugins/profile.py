...
async def _(event):
    ...
    if photo:
        ...
        try:
            ...
            await eor(nobi, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eor(nobi, result)
    except Exception as e:
        await parse_error(nobi, e)


@nobi_cmd(pattern="online$")
async def _(event):
    ForGo10God, nobi_USER, _ = await client_id(event)
    user = await event.client.get_entity(ForGo10God)
    if nobi_USER.startswith("[ • OFFLINE • ]"):
        nobi = await eor(event, "**Changing Profile to Online...**")
    else:
    ...
    if photo:
        ...
        try:
            ...
            await eor(nobi, "**Changed profile to Online.**")
    try:
        os.remove("donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@nobi_cmd(pattern="ppic$")
async def _(event):
    reply = await event.get_reply_message()
    nobi = await eor(event, "Changing profile picture ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            ...
            try:
                ...
                await eod(nobi, "**📍 Profile picture changed successfully.**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.warn(str(e))
...
@nobi_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await nobi.edit(result, parse_mode="HTML")


@nobi_cmd(pattern="delpfp(?:\s|$)([\s\S]*)")
...
@nobi_cmd(pattern="myusernames$")
async def _(event):
    _, _, nobi_mention = await client_id(event)
    result = await event.client(GetAdminedPublicChannelsRequest())
    output_str = f"**Usernames reserved by:** {nobi_mention}\n\n"
    for channel_obj in result.chats:
        output_str += f"• {channel_obj.title} ~ @{channel_obj.username} \n"
    await eor(event, output_str)


@nobi_cmd(pattern="stats$")
async def stats(event):
    nobi = await eor(event, "`Collecting stats...`")
    ...
    response = f"<b><i><u>♛ Stats for {nobi_mention} ♛</b></i></u>\n\n"
    ...
    await nobi.edit(response, parse_mode="HTML", link_preview=False)
