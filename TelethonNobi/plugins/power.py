...
import heroku3
from TelethonNobi.DB.gvar_sql import addgvar, delgvar, gvarstat
...
async def restart(event):
    if Config.HEROKU_APP_NAME and Config.HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted 𝕹𝕺𝕭𝕴 𝖀𝕭✍** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@nobi_cmd(pattern="restart$")
async def re(nobi):
    event = await eor(nobi, "Restarting 𝕹𝕺𝕭𝕴 𝖀𝕭✍ ...")
    try:
        await restart(event)
...
@nobi_cmd(pattern="reload$")
async def rel(event):
    await eor(event, "**Reloaded NobiUb!** \n\n__This might take a minute.__")
    await reload_NobiUb()


@nobi_cmd(pattern="shutdown$")
async def down(event):
    await eor(event, "**[ ⚠️ ]** \n**𝕹𝕺𝕭𝕴 𝖀𝕭✍ is now turned off. Manually turn it on to start again.**")
...
async def sett(event):
    ...
    val = lists[2].strip()
    nobi = await eor(event, f"**Setting variable** `{var}` **as** `{val}`")
    if var == "":
        return await parse_error(nobi, f"__Invalid Syntax !!__ \n__Try:__ `{hl}svar VARIABLE_NAME variable_value`", False)
    elif val == "":
        return await parse_error(nobi, f"__Invalid Syntax !!__ \n__Try:__ `{hl}svar VARIABLE_NAME variable_value`", False)
    if var not in db_config:
        return await parse_error(nobi, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    try:
        addgvar(var, val)
    except Exception as e:
        return await parse_error(nobi, e)
    await eod(nobi, f"**Variable Added Successfully!!** \n\n**• Variable:** `{var}` \n**» Value:** `{val}`")


@nobi_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
async def gett(event):
    lists = event.text.split(" ", 2)
    if len(lists) < 2:
        return await parse_error(event, f"__Invalid Syntax !!__ \n__Try:__ `{hl}gvar VARIABLE_NAME`", False)
    var = lists[1].strip().upper()
    nobi = await eor(event, f"**Getting variable** `{var}`")
    if var == "":
        return await parse_error(nobi, f"__Invalid Syntax !!__ \n__Try:__ `{hl}gvar VARIABLE_NAME`", False)
    if var not in db_config:
        return await parse_error(nobi, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    try:
        ...
        return await parse_error(nobi, e)
    await nobi.edit(f"**• OS VARIABLE:** `{var}`\n**» OS VALUE :** `{os_v}`\n\n**• DB VARIABLE:** `{var}`\n**» DB VALUE :** `{db_v}`\n")


@nobi_cmd(pattern="dvar(?:\s|$)([\s\S]*)")
async def dell(event):
    lists = event.text.split(" ", 2)
    if len(lists) < 2:
        return await parse_error(event, f"__Invalid Syntax !!__ \n__Try:__ `{hl}dvar VARIABLE_NAME`", False)
    var = lists[1].strip().upper()
    nobi = await eor(event, f"**Deleting Variable** `{var}`")
    if var == "":
        return await parse_error(nobi, f"__Invalid Syntax !!__ \n__Try:__ `{hl}dvar VARIABLE_NAME`", False)
    if var not in db_config:
        return await parse_error(nobi, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    if gvarstat(var):
        try:
            x = gvarstat(var)
            delgvar(var)
            await eod(nobi, f"**Deleted Variable Successfully!!** \n\n**• Variable:** `{var}` \n**» Value:** `{x}`")
        except Exception as e:
            await parse_error(nobi, e)
    else:
        await parse_error(nobi, f"`{var}` __does not exists.__", False)
...
CmdHelp("power").add_command(
    ...
    "shutdown", None, "Turns off 𝕹𝕺𝕭𝕴 𝖀𝕭✍. Userbot will stop working unless you manually turn it on."
).add_command(
