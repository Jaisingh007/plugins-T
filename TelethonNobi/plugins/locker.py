...
@nobi_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    nobi_mention = cid[2]
    nobi = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await parse_error(nobi, "Nothing given to lock in chat.")
    locker = lock(text)
    if not locker:
        return await eod(
            nobi,
    ...
    await event.client.send_file(
        event.chat_id,
        restlo,
        caption=f"{nobi_mention} Locked `{text}` \n__Cause its Rest Time !!__",
    )
    await nobi.delete()


@nobi_cmd(pattern="unlock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[8:]
    cid = await client_id(event)
    nobi_mention = cid[2]
    nobi = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await parse_error(nobi, "Nothing given to unlock in chat.")
    ...
    await event.client.send_file(
        event.chat_id, shuru, caption=f"**{nobi_mention} unlocked** `{text}`"
    )
    await nobi.delete()


@nobi_cmd(pattern="ltype$")
async def _(event):
    await eor(
        event,
