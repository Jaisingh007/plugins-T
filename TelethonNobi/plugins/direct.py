...
@nobi_cmd(pattern="direct(?:\s|$)([\s\S]*)")
async def direct_link_generator(request):
    if request.fwd_from:
        return
    nobievent = await eor(request, "`Processing...`")
    ...
    if message:
        ...
        await nobievent.edit(f"**Usage :** {hl}direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await nobievent.edit(reply)
    for link in links:
...
def cm_ru(url: str) -> str:
    ...
    Using https://github.com/JrnobiModelBuilder/cmrudl.py"""
    reply = ""
    try:
