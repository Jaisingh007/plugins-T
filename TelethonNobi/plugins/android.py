...
@nobi_cmd(pattern="app(?:\s|$)([\s\S]*)")
async def apk(event):
    lists = event.text.split(" ", 1)
    if not len(lists) == 2:
        return await parse_error(event, "Invalid syntax.")
    app_name = lists[1].strip()
    nobi = await eor(event, f"__Searching for__ `{app_name}` __...__")
    try:
        ...
        await nobi.edit(app_details, link_preview=True, parse_mode="HTML")
    except IndexError:
        await parse_error(nobi, "No result found in search. Please enter **Valid app name**", False)
    except Exception as err:
        await parse_error(nobi, err)


@nobi_cmd(pattern="magisk$")
async def _(magisk):
    if magisk.fwd_from:
        return
    magisk_repo = "https://raw.githubusercontent.com/topjohnwu/magisk_files/"
    magisk_dict = {
        "⦁ **Stable**": magisk_repo + "nobi/stable.json",
        "⦁ **Beta**": magisk_repo + "nobi/beta.json",
...
async def device_info(request):
    ...
    data = json.loads(
        get(
            ...
            "certified-android-devices/nobi/by_device.json"
        ).text
    )
...
@nobi_cmd(pattern="codename(?: |)([\S]*)(?: |)([\s\S]*)")
async def codename_info(request):
    ...
    data = json.loads(
        get(
            "https://raw.githubusercontent.com/androidtrackers/"
            "certified-android-devices/nobi/by_brand.json"
        ).text
    )
