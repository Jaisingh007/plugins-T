...
@nobi_cmd(pattern="img(?:\s|$)([\s\S]*)")
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await parse_error(event, "Nothing given to search.")
    nobi = await eor(event, f"Searching for `{sim}`...")
    if ";" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@nobi_cmd(pattern="reverse(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    link = alls["href"]
    text = alls.text
    await nobi.edit(f"**Possible Results:** [{text}](google.com{link})")
...
async def gps(event):
    ...
    nobi = await eor(event, "Finding😁")
    geolocator = Nominatim(user_agent="NobiUb")
...
async def _(event):
    ...
    try:
        ...
        await nobi.edit("Starting Google Chrome BIN")
        driver = webdriver.Chrome(chrome_options=chrome_options)
        input_str = event.pattern_match.group(1)
        driver.get(input_str)
        await nobi.edit("Calculating Page Dimensions")
        ...
        await nobi.edit("Painting web-page")
        driver.set_window_size(width + 100, height + 100)
        im_png = driver.get_screenshot_as_png()
        driver.close()
        await nobi.edit("Stopping Google Chrome BIN")
        ...
        with io.BytesIO(im_png) as out_file:
            out_file.name = "nobi_Capture.PNG"
            await event.client.send_file(
                event.chat_id,
                out_file,
                caption=caption,
        ...
        await parse_error(nobi, traceback.format_exc())


@nobi_cmd(pattern="cricket$")
async def _(event):
...
CmdHelp("google").add_command(
    ...
    "webshot", "<link>", f"Gives out the web screenshot of given link via Google Crome Bin in .png format", "webshot https://github.com/jaisingh007/NobiUb"
).add_command(
