...
def button(page, modules):
    ...
    for pairs in pairs[page]:
        buttons.append(
            [
                Button.inline(f"{nobi_emoji} {pair} {nobi_emoji}", data=f"Information[{page}]({pair})")
                for pair in pairs
            ]
        )
    ...
    buttons.append(
        [
            Button.inline(f"⤟ Back {nobi_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"),
            Button.inline(f"• ❌ •", data="close"),
            Button.inline(f"{nobi_emoji} Next ⤠", data=f"page({0 if page == (max_pages - 1) else (page + 1)})"),
        ]
    )
...
if Config.BOT_USERNAME and tbot:
    ...
    async def inline_handler(event):
        ForGo10God, nobi_USER, nobi_mention = await client_id(event, event.query.user_id)
        builder = event.builder
        result = None
        query = event.text
        auth = await clients_list()
        if event.query.user_id in auth and query == "NobiUb_help":
            ...
            alive_name = gvarstat("ALIVE_NAME") or nobi_USER
            he_ll = alive_txt.format(
                alive_msg, telethon_version, NobiUb_version, uptime, abuse_m, is_sudo
            )
            alv_btn = [
                [
            ...
            nobi_FIRST = f"🔥 𝙃𝙚𝙡𝙡𝘽𝙤𝙩 𝙋𝙈 𝙎𝙚𝙘𝙪𝙧𝙞𝙩𝙮 🔥\n\nHello!! This is an automated message on behalf of {nobi_mention}."
            if CSTM_PMP:
                nobi_FIRST += f"\n\n{CSTM_PMP}"
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚡ ʟɛɢɛռɖaʀʏ ᴀғ 𝕹𝕺𝕭𝕴 𝖀𝕭 ⚡**",
                buttons=[
                    [Button.url("📑 Repo 📑", "https://github.com/jaisingh007/THE-NOBI_UB")],
                    [Button.url("NobiUb Network", "https://t.me/NobiUb_networks")],
                ],
            )
            ...
            result = builder.article(
                "@THE_NOBIUB",
                text="""**Hey! This is [𝕹𝕺𝕭𝕴 𝖀𝕭✍](https://t.me/THE_NOBIUB) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        Button.url("• Updates •", "https://t.me/THE_NOBIUB"),
                        Button.url("• Chat •", "https://t.me/NobiUb_chats"),
                    ],
                    [
                        Button.url("• Repo •", "https://github.com/jaisingh007/THE-NOBI_UB"),
                        Button.url("• Docs •", "https://NobiUb.tech"),
                    ],
                    [
                        Button.url("◈ NobiUb Network ◈", "https://t.me/NobiUb_networks"),
                    ],
    ...
    async def on_pm_click(event):
        ...
        if event.query.user_id in auth:
            ...
            reply_popup = "🔰 This is 𝕹𝕺𝕭𝕴 𝖀𝕭✍ PM Security to keep away unwanted retards from spamming PM !!"
    ...
    async def on_pm_click(event):
        ...
        if event.query.user_id in auth:
            ...
            await event.edit(
                "✅ **Request Registered** \n\nMy nobi will now decide to look for your request or not.\n😐 Till then wait patiently and don't spam!!"
            )
    ...
    async def reopn(event):
        _, _, nobi_mention = await client_id(event, event.query.user_id)
        auth = await clients_list()
        if event.query.user_id in auth:
            ...
            await event.answer("Hello! This help menu is not for you, you can make yourself a NobiUb and use your bot. Go to @THE_NOBIUB for more info.", cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if event.query.user_id in auth:
            veriler = Button.inline(
                f"{nobi_emoji} Re-Open Menu {nobi_emoji}", data="reopen"
            )
            await event.edit(
                f"**🎭 Closed NobiUb's help menu**\n\n**Bot Of:**  {nobi_mention}\n\n        [©️ 𝕹𝕺𝕭𝕴 𝖀𝕭✍ ™️]({chnl_link})",
                buttons=veriler,
                link_preview=False,
            )
        else:
    ...
    async def send(event):
        ...
        _, _, nobi_mention = await client_id(event, event.query.user_id)
        omk = f"**• Plugin name ≈** `{plugin}`\n**• Uploaded by ≈** {nobi_mention}\n\n⚡ **[ʟɛɢɛռɖaʀʏ ᴀғ 𝕹𝕺𝕭𝕴 𝖀𝕭]({chnl_link})** ⚡"
        the_plugin_file = "./TelethonNobi/plugins/{}.py".format(plugin.lower())
        butt = Button.inline(f"{nobi_emoji} Main Menu {nobi_emoji}", data="reopen")
    ...
    async def page(event):
        ...
        if event.query.user_id in auth:
            ...
            return await event.answer(
                "Hello! This help menu is not for you, you can make yourself a NobiUb and use your bot. Go to @THE_NOBIUB for more info.",
                cache_time=0,
                alert=True,
            )
    ...
    async def Information(event):
        ...
        buttons.append([Button.inline(f"{nobi_emoji} Main Menu {nobi_emoji}", data=f"page({page})")])
        if event.query.user_id in auth:
            await event.edit(
    ...
    async def commands(event):
        ...
        if event.query.user_id in auth:
            await event.edit(
                ...
                buttons=[
                    Button.inline(
                        f"{nobi_emoji} Return {nobi_emoji}",
