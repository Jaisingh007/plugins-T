...
@nobi_cmd(pattern="create (b|g|c) ([\s\S]*)")
async def _(event):
    type_of_group = event.pattern_match.group(1)
    group_name = event.pattern_match.group(2)
    nobi = await eor(event, "Creating ...")
    if type_of_group == "b":
        try:
        ...
        await nobi.edit(f"Read `{hl}plinfo create` to know how to use me")


@nobi_cmd(pattern="link ([\s\S]*)")
