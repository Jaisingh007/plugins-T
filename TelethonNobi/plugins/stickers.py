...
from TelethonNobi.DB.gvar_sql import addgvar, gvarstat
from TelethonNobi.plugins import *


@nobi_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    nobi = await eor(event, "__**Starting sticker kang process ...**__")
    ...
    ForGo10God, nobi_USER, _ = await client_id(event)
    nick = f"@{user.username}" if user.username else nobi_USER
    name = user.username if user.username else ForGo10God
    custompack = gvarstat("STICKER_PACKNAME")
    packname = f"NobiUb_{name}_{Sticker.pack}"
    packnick = f"{custompack}" if custompack else f"{nick}'s 𝕹𝕺𝕭𝕴 𝖀𝕭✍ Vol.{Sticker.pack}"
    is_sta = False
    is_ani = False
    is_vid = False
    photo = None
    ...
    if reply and reply.media:
        if reply.photo:
            ...
            if reply.media.document.mime_type == "video/webm":
                await nobi.edit("__Oow! A video sticker ...__ **[ ENCODING ]**")
                ...
                await nobi.edit("__Oow! A video ...__ **[ CONVERTING ]**")
                await VSticker(event, reply)
            is_vid = True
            photo = 1
        else:
            return await eod(nobi, "__Can't kang that 🔪__")
    else:
        return await eod(nobi, "__Can't kang that 🔪__")
    await nobi.edit("__**Adding this sticker to your pack...**__")
        ...
        async with event.client.conversation("@Stickers") as conv:
            try:
                ...
                while ("50" in resp.message) or ("120" in resp.message) or is_vid:
                    ...
                    packname = f"NobiUb_{name}_{Sticker.pack}"
                    packnick = f"{custompack}" if custompack else f"{nick}'s 𝕹𝕺𝕭𝕴 𝖀𝕭✍ Vol.{Sticker.pack}"
                    await nobi.edit(f"__**Switching pack due to insufficient space ...**__ \n__Pack:__ `{Sticker.pack}`")
                    await conv.send_message(packname)
                    resp = await conv.get_response()
                    if resp.message == "Invalid set selected.":
                        ...
                        if "Sorry, the file type is invalid." in rsp.text:
                            return await eod(nobi, "`Failed to add sticker, use` @Stickers `bot to add the sticker manually.`")
                ...
                return await parse_error(nobi, "__Unblock @Stickers and try again.__", False)
    else:
        await nobi.edit("__**Preparing a new pack....**__")
        async with event.client.conversation("@Stickers") as conv:
            await conv.send_message(cmd)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
    ...
    await tbot.send_message(
        Config.LOGGER_ID,
        f"#KANG #STICKER \n\nA sticker has been kanged into the pack of {nobi_USER}. Click below to see the pack!",
        buttons=[
          [Button.url("View Pack", f"t.me/addstickers/{packname}")],
          [Button.url(nobi_USER, f"tg://user?id={ForGo10God}")],
        ],
        parse_mode=None,
    )
    await eod(
...
@nobi_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    nobi = await eor(event, "`Preparing pack kang...`")
    reply = await event.get_reply_message()
    ForGo10God, nobi_USER, _ = await client_id(event)
    ...
    user = await event.client.get_me()
    un = f"@{user.username}" if user.username else nobi_USER
    un_ = user.username if user.username else ForGo10God
    if not reply:
        return await eod(nobi, "`Reply to a stciker to kang that pack.`")
    if len(lists) == 1:
        pname = f"{un}'s 𝕹𝕺𝕭𝕴 𝖀𝕭✍ Pack"
    else:
        pname = lists[1].strip()
    if reply and reply.media and reply.media.document.mime_type == "image/webp":
        nobi_id = reply.media.document.attributes[1].stickerset.id
        nobi_hash = reply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await event.client(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=nobi_id, access_hash=nobi_hash)
            )
        )
        stcrs = []
        ...
        if x is None:
            ...
            pack = int(x) + 1
        await nobi.edit("`Starting kang process...`")
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
                    title=pname,
                    short_name=f"nobi_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
            addgvar("PKANG", pack)
            ...
            await nobi.edit("`Pack name already occupied... making new pack`")
            pack = int(pack) + 1
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
        ...
        await tbot.send_message(
            Config.LOGGER_ID,
            f"#PKANG #STICKER \n\nA sticker pack has been kanged by {nobi_USER}. Click below to see the pack!",
            buttons=[
        ...
        await nobi.edit("Unsupported File. Please Reply to a sticker only.")
    await to_del.delete()


@nobi_cmd(pattern="stkrinfo$")
async def get_pack_info(event):
    ...
    try:
        ...
        nobi = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    ...
    await nobi.edit(OUTPUT, parse_mode='HTML')


@nobi_cmd(pattern="delst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "Reply to a sticker to delete it.")
    nobi = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            return await parse_error(nobi, "__Unblock @Stickers and try again.__", False)
        await nobi.edit(fourth.text)


@nobi_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
    ...
    emoji = lists[1].strip()
    nobi = await eor(event, f"📝 `Editing sticker emoji to {emoji}`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            if fourth.text.startswith("Current emoji:"):
                ...
                return await eod(nobi, "That's not your sticker!")
        except YouBlockedUserError:
            return await parse_error(nobi, "__Unblock @Stickers and try again.__", False)
        await nobi.edit(f"{sixth.text}")


@nobi_cmd(pattern="text(?:\s|$)([\s\S]*)")
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@nobiFonts")
