...
async def slap(replied_user, event):
    ...
    nobi_mention = cid[2]
    caption = temp.format(
        user1=nobi_mention, user2=slapped, item=item, hits=hit, throws=throw
    )
    return caption


@nobi_cmd(pattern=f"randi$")
async def rendi(e):
    txt = random.choice(RENDISTR)
    await eor(e, txt)


@nobi_cmd(pattern=f"habuse$")
async def thenus(e):
    txt = random.choice(THANOS_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"fuk$")
async def tapatap(e):
    txt = random.choice(FUK_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"chu$")
async def chut(e):
    txt = random.choice(CHU_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"noob$")
async def nub(e):
    txt = random.choice(NOOBSTR)
    await eor(e, txt)


@nobi_cmd(pattern=f"run$")
async def metoo(e):
    txt = random.choice(RUNSREACTS)
    await eor(e, txt)


@nobi_cmd(pattern=f"gali$")
async def metoo(e):
    txt = random.choice(GAALI_STR)
    await eor(e, txt)


@nobi_cmd(pattern=f"rape$")
async def metoo(e):
    txt = random.choice(RAPE_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"abuse$")
async def metoo(e):
    txt = random.choice(ABUSE_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"gey$")
async def metoo(e):
    txt = random.choice(GEY_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"piro$")
async def metoo(e):
    txt = random.choice(PRO_STRINGS)
    await eor(e, txt)


@nobi_cmd(pattern=f"insult$")
async def metoo(e):
...
@nobi_cmd(pattern="cry$")
async def cry(e):
    txt = random.choice(CRI)
    await eor(e, txt)


@nobi_cmd(pattern="cp(?:\s|$)([\s\S]*)")
...
@nobi_cmd(pattern="react$")
async def react_meme(react):
    await eor(react, random.choice(FACEREACTS))


@nobi_cmd(pattern="clap(?:\s|$)([\s\S]*)")
