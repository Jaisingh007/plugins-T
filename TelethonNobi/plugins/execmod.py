...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**𝕹𝕺𝕭𝕴 𝖀𝕭✍'s Environment Module :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**𝕹𝕺𝕭𝕴 𝖀𝕭✍'s Server Speed Calculated :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
CmdHelp("execmod").add_command(
    ...
    "env", None, "Shows Environment veriables of your NobiUb"
).add_command(
    "speed", None, "Shows server speed of your NobiUb"
).add_info(
    "Exec Modules."
).add_warning(
    "✅ Harmless Module."
