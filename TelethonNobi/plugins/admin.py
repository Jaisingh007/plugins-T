...
from TelethonNobi.DB.mute_sql import is_muted, mute, unmute
...
@nobi_cmd(pattern="promote(?:\s|$)([\s\S]*)")
@errors_handler
async def promote(event):
    ...
    nobievent = await eor(event, "`Promoting User...`")
...
async def demote(event):
    ...
    if not admin and not creator:
        return await parse_error(event, NO_ADMIN)
    nobievent = await eor(event, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        return await parse_error(nobievent, NO_PERM)
    await nobievent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{event.chat.title}`")
    await event.client.send_message(
        Config.LOGGER_ID,
        "#DEMOTE\n"
        f"\n**USER:** [{user.first_name}](tg://user?id={user.id})"
...
@nobi_cmd(pattern="mute(?:\s|$)([\s\S]*)")
async def muth(event):
    x = await client_id(event)
    ForGo10God = x[0]
    if event.is_private:
        nobi = await eor(event, "**Enough of your bullshit  !!**")
        await event.get_reply_message()
        if is_muted(event.chat_id, event.chat_id):
            return await eod(nobi, "User is already muted here 🥴")
        if event.chat_id == ForGo10God:
            return await parse_error(nobi, "You can't mute yourself !")
        try:
            mute(event.chat_id, event.chat_id)
            await eod(nobi, "**Muted this user !**")
        except Exception as e:
            return await parse_error(nobi, e)
    elif event.is_group:
        nobi = await eor(event, "`Muting...`")
        ...
        if event.reply_to_msg_id:
            ...
            if input_str.isdigit():
                try:
                    ...
                    return await parse_error(nobi, ve)
            else:
                userid = (await event.client.get_entity(input_str)).id
                name = (await event.client.get_entity(userid)).first_name
        else:
            return await parse_error(nobi, "I need an user to mute !!")
        if userid == ForGo10God:
            return await parse_error(nobi, "You can't mute yourself !")
        if str(userid) in DEVLIST:
            return await parse_error(nobi, "Cant mute my developers !")
        if ForGo10God not in admin_:
            return await parse_error(nobi, NO_PERM)
        if userid in admin_:
            if is_muted(userid, event.chat_id):
                return await eod(nobi, "Admin already muted ♪～(´ε｀ )")
            try:
...
@nobi_cmd(pattern="unmute(?:\s|$)([\s\S]*)")
async def nomuth(event):
    x = await client_id(event)
    ForGo10God = x[0]
    nobi = await eor(event, "`Unmuting ...`")
    if event.is_private:
        if not is_muted(event.chat_id, event.chat_id):
            return await eod(nobi, "Not even muted !!")
        try:
            unmute(event.chat_id, event.chat_id)
            await eod(nobi, "User unmuted successfully !")
        except Exception as e:
            await parse_error(nobi, e)
    elif event.is_group:
        input_str = event.pattern_match.group(1)
        ...
        if event.reply_to_msg_id:
            ...
            return await parse_error(nobi, "I need a user to unmute!!")
        if ForGo10God not in admin_:
            return await parse_error(nobi, NO_PERM)
        if userid in admin_:
...
@nobi_cmd(pattern="ban(?:\s|$)([\s\S]*)")
@errors_handler
async def ban(event):
    nobievent = await eor(event, "`Banning Nigga...`")
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
    ...
    if not user:
        return await parse_error(nobievent, "`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await eod(nobievent, "**Say again? Ban my creator??**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        await parse_error(nobievent, NO_PERM)
        return
    try:
        reply = await event.get_reply_message()
        if reply:
...
async def nothanos(event):
    ...
    if not admin and not creator:
        await parse_error(event, NO_ADMIN)
        return
    nobievent = await eor(event, "`Unbanning...`")
    user, _ = await get_user_from_event(event)
    user = user[0]
    if not user:
        return
    try:
        ...
        await parse_error(nobievent, "Invalid UserId!! Please Recheck it!!")


@nobi_cmd(pattern="pin(?:\s|$)([\s\S]*)")
@errors_handler
async def pin(event):
    chat = await event.get_chat()
...
async def kick(event):
    ...
    nobievent = await eor(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await parse_error(nobievent, e)
        return
    if reason:
        await nobievent.edit(
...
@nobi_cmd(pattern="zombies(?:\s|$)([\s\S]*)")
async def rm_deletedacc(event):
    ...
    if action == "clean":
        ...
        if not admin and not creator:
            return await parse_error(event, NO_ADMIN)
        nobi = await eor(event, "__🧹 Purging Zombies from here ...__")
        del_u = 0
        del_a = 0
...
#                     await sleep(0.5)
        ...
        nobi = await eor(event, "**Searching For Zombies...**")
        async for user in event.client.iter_participants(event.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(0.5)
