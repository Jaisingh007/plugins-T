...
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        ...
        try:
            ...
            result = _out_
        nobiuser = await event.client.get_me()
        if nobiuser.username:
            curruser = nobiuser.username
        else:
            curruser = "@THE_NOBIUB"
        uid = os.geteuid()
        if uid == 0:
...
@nobi_cmd(pattern="eval(?:\s|$)([\s\S]*)")
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        cmd = "".join(event.text.split(maxsplit=1)[1:])
        if not cmd:
            return await eod(event, "`What should i run ?..`")
        nobievent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec", "<command>", "Execute a Terminal command on NobiUb server and shows details"
).add_command(
