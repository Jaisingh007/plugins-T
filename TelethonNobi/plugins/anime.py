...
@nobi_cmd(pattern="anime(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        return await eor(event, "Please give anime name to search on Anilist.")
    nobi = await eor(event, f"__Searching for__ `{query}` __on Anilist.__")
    qdb = rand_key()
    ANIME_DB[qdb] = query
    result = await get_anilist(qdb, 1)
    if len(result) == 1:
        return await nobi.edit(result[0])
...
@nobi_cmd(pattern="manga(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        await eor(event, "Please give manga name to search..")
    nobi = await eor(event, f"__Searching for__ `{query}` ...")
    qdb = rand_key()
    MANGA_DB[qdb] = query
    ...
    try:
        ...
        await nobi.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@nobi_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
    ...
    try:
        ...
        await nobi.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@nobi_cmd(pattern="fillers(?:\s|$)([\s\S]*)")
async def canon(event):
    nobi = event.text[9:]
    if nobi == "":
        return await eor(event, "`Give anime name to search filler episodes.`")
    nub = await eor(event, f"Searching Filler Episodes For `{nobi}`")
    hel_ = search_filler(nobi)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{nobi}`")
    ...
    for i in list_:
        ...
        NobiUb += f"• [{i}]({paste})\n"
    await nub.edit(NobiUb)


@nobi_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    nobi = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(nobi, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
...
@nobi_cmd(pattern="aniuser(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[9:]
    nobi = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await nobi.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(nobi, result[0])
    pic, msg = result
    try:
        await event.client.send_file(
            event.chat_id,
...
CmdHelp("anime").add_command(
    ...
    "aniuser", "<anilist username>", "Searches for the Anilist Stats of the given user.", "aniuser meiznobiboy"
).add_command(
