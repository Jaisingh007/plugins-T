...
async def _(event):
    ...
    nobi = await eor(event, "Making Telegraph Link....")
    ...
    if reply:
        ...
        await eod(nobi, "Reply to a message to get a permanent telegra.ph link.")


@nobi_cmd(pattern="tgraph(?:\s|$)([\s\S]*)")
async def _(event):
    nobi = await eor(event, "Making telegraph post ...")
    reply = await event.get_reply_message()
    lists = event.text.split(" ", 1)
    if not reply:
