...
@nobi_cmd(pattern="thug$")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to a image...")
    nobi = await eor(event, "`Converting To thug Image..`")
    await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    imagePath = img
    maskPath = "./nobiConfig/resources/pics/mask.png"
    cascPath = "./nobiConfig/resources/xmls/haarcascade_frontalface_default.xml"
...
@nobi_cmd(pattern="trigger$")
async def _(event):
    nobi = await eor(event, "`Trigggggggggerrr`")
...
@nobi_cmd(pattern="geyy$")
async def _(event):
    nobi = await eor(event, "`Geyyyy`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://te.legra.ph{url_s[0]}"
...
@nobi_cmd(pattern="pix$")
async def _(event):
    nobi = await eor(event, "`Pixing This Image.`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://te.legra.ph{url_s[0]}"
...
@nobi_cmd(pattern="ytc(?:\s|$)([\s\S]*)")
async def _(event):
    nobi = await eor(event, "`Making a youthuub comment...`")
    owo = await event.get_reply_message()
...
@nobi_cmd(pattern="audiomeme(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if meme:
        await event.delete()
        nobi = await meme[0].click(Config.LOGGER_ID)
        if nobi:
            await event.client.send_file(
                event.chat_id,
                nobi,
