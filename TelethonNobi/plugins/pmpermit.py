...
from TelethonNobi.DB import pmpermit_sql as pm_sql
...
async def do_pm_permit_action(chat_ids, event, client):
    ...
    nobi = await tap[0].click(event.chat_id)
    client.PM_WARNS[chat_ids] += 1
    if chat_ids in client.PREV_REPLY_MESSAGE:
        await client.PREV_REPLY_MESSAGE[chat_ids].delete()
    client.PREV_REPLY_MESSAGE[chat_ids] = nobi
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by 𝕹𝕺𝕭𝕴 𝖀𝕭✍"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
