...
async def all_groups_id(nobi):
    nobigroups = []
    async for dialog in nobi.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            nobigroups.append(entity.id)
    return nobigroups


@nobi_cmd(pattern="frwd$")
async def _(event):
