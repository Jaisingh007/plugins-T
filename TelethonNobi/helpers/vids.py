...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            nobiresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            nobifile = await event.client.download_media(nobiresponse, "./temp")
            return await unzip(nobifile)
