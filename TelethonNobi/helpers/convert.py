...
from TelethonNobi.helpers.progress import progress
from TelethonNobi.helpers.runner import runcmd
from TelethonNobi.helpers.vids import take_screen_shot
from TelethonNobi.utils.extras import edit_or_reply as eor
...
async def convert_to_image(event, client):
    nobi = await event.get_reply_message()
    if not (
        nobi.gif
        or nobi.audio
        or nobi.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if nobi and nobi.photo:
        nobi_final = downloaded_file_name
    elif nobi.sticker and nobi.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        nobi_final = image_name20
    elif nobi.sticker and nobi.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        nobi_final = imgpath
    elif nobi.gif or nobi.video or nobi.video_note:
...
async def tgs_to_gif(file, tgs=False, video=False):
    if tgs:
        cmd = f"lottie_convert.py '{file}' 'NobiUb.gif'"
    elif video:
        cmd = f"ffmpeg -i '{file}' -c copy 'NobiUb.gif'"
    await runcmd(cmd)
    return 'NobiUb.gif'
